...
-- Reuse a single drawing text object for text bounds reads.
local text_measure = Drawing.new("Text")
...
function utility.dragify(main, dragoutline, object, allow_tween)
    ...
    utility.connect(services.InputService.InputChanged, function(input)
        if input.UserInputType == UserInputType.MouseMovement and dragging then
    ...
    utility.connect(services.InputService.InputEnded, function(input)
        if input.UserInputType == UserInputType.MouseButton1 and dragging then 
            dragging = false
            dragoutline.Visible = false
            if allow_tween then
                local pos = tween.new(object, TweenInfo.new(settings.drag_time, EasingStyle[settings.drag_easying_style], EasingDirection[settings.drag_easying_direction]), {Position = currentpos})
                pos:Play()
            else
...
function utility.resize(object, background, dragoutline)
    pcall(function()
        ...
        local currentsize = background.Size
        local vector2size = Vector2.new(currentsize.X.Offset, currentsize.Y.Offset)
        object.MouseButton1Down:Connect(function(input)
            dragging = true
            start = input
            dragoutline.Visible = true
            objectposition = background.Size
        end)
        utility.connect(services.InputService.InputChanged, function(input)
            if input.UserInputType == UserInputType.MouseMovement and dragging then
                local MouseLocation = services.InputService:GetMouseLocation()
...
function utility.textlength(str, font, fontsize)
    ...
    text_measure.Font = font
    text_measure.Size = fontsize
    return text_measure.TextBounds
end
...
local allowedcharacters_lookup = {}
...
for i = 32, 126 do
    ...
    table.insert(allowedcharacters, ch)
    allowedcharacters_lookup[ch] = true
end
local pickers = {}
            ...
            connection = utility.connect(services.InputService.InputBegan, function(input)
                if input.UserInputType == Enum.UserInputType.Keyboard then
                    if input.KeyCode ~= Enum.KeyCode.Backspace then
                        ...
                        if allowedcharacters_lookup[str] then
                            keyqueue = keyqueue + 1
                            local currentqueue = keyqueue
                            local shiftDown = services.InputService:IsKeyDown(Enum.KeyCode.RightShift) or services.InputService:IsKeyDown(Enum.KeyCode.LeftShift)
                            local charToAppend = shiftDown and (shiftcharacters[str] or str:upper()) or str:lower()

                            text.Text = text.Text .. charToAppend
                            callback(text.Text)
...
local function normalizeDropdownDefault(default, options, max)
    ...
    if type(default) == "table" then
        if max then
            for i, opt in next, default do
                if not table.find(options, opt) or i > max then
    ...
    elseif default ~= nil and not table.find(options, default) then
        default = nil
    end
...
function library.object_slider(parent, cfg)
    local slider_tbl = {}
    ...
    local holder = utility.create("Square", {Parent = parent, Visible = true, Transparency = 0, Size = UDim2.new(1,0,0,15), Thickness = 1, Filled = true, ZIndex = 7})
    ...
    utility.create("Text", {Text = name, Parent = holder, Visible = true, Transparency = 1, Color = Color3.new(1,1,1), Size = 13, Center = false, Outline = false, Font = Drawing.Fonts.Plex, Position = UDim2.new(0,155,0,0), ZIndex = 7})
    utility.create("Text", {Text = name, Parent = holder, Visible = true, Transparency = 1, Color = Color3.new(0,0,0), Size = 13, Center = false, Outline = false, Font = Drawing.Fonts.Plex, Position = UDim2.new(0,155,0,1), ZIndex = 6})
    ...
    local function set(value)
        ...
        callback(value)
        slider_drag_bar.Visible = sizeX >= 0.11
    end
...
function library.object_keybind(parent, cfg)
    ...
    local holder = utility.create("Square", {Transparency = 0, ZIndex = 7, Size = UDim2.new(1, 0, 0, 15), Parent = parent})
    if allow_tool then
        tooltip(cfg, tooltip_text, holder)
    end
    utility.create("Text", {
    ...
    local function set(newkey)
        ...
        if newkey ~= nil and not table.find(blacklist, newkey) then
            key = newkey
            local t = (keys[newkey] or tostring(newkey):gsub("Enum.KeyCode.", ""))
            keytext.Text = t
        else
            key = nil
            keytext.Text = ""
    ...
    utility.connect(services.InputService.InputBegan, function(inp)
        if (inp.KeyCode == key or inp.UserInputType == key) and not binding then
            if key_mode == "Hold" then
                ...
                c = utility.connect(RunService.RenderStepped, function()
                    if callback then
                        callback(true)
                    end
                end)
...
function library.object_textbox_wrap(parent, cfg)
    local textbox_tbl = {}
    local placeholderStr = cfg.placeholder or cfg.Placeholder or "new textbox"
    ...
    local phLabel = utility.create("Text", {
        Text = placeholderStr,
        Font = Drawing.Fonts.Plex,
        Transparency = 0.5,
        Size = 13,
...
function library.object_colorpicker_wrap(parent, cfg)
...
function library.object_button(parent, cfg)
    local button_tbl = {}
...
function library.object_dropdown(holder, content, flag, callback, default, max, scrollable, scrollingmax, section)
        ...
        button.MouseButton1Click:Connect(function()
            if max then
                if table.find(chosen, option) then
                    table_remove(chosen, table_find(chosen, option))

                    local textchosen = {}
                    local cutobject = false

                    for _, opt in next, chosen do
                        table_insert(textchosen, opt)

                        if utility.textlength(table_concat(textchosen, ", ") .. ", ...", Drawing.Fonts.Plex, 13).X > (dropdown.AbsoluteSize.X - 18) then
                            cutobject = true
                            table_remove(textchosen, #textchosen)
                        end
                    end

                    value.Text = #chosen == 0 and "" or table_concat(textchosen, ", ") .. (cutobject and ", ..." or "")
                    ...
                    if #chosen == max then
                        ...
                        table_remove(chosen, 1)
                    end

                    table_insert(chosen, option)
    ...
    set = function(option)
        if max then
            ...
            for opt, tbl in next, optioninstances do
                if not table_find(option, opt) then
            ...
            for i, opt in next, option do
                if table_find(content, opt) and #chosen < max then
                    table_insert(chosen, opt)
            ...
            for _, opt in next, chosen do
                ...
                if utility.textlength(table_concat(textchosen, ", ") .. ", ...", Drawing.Fonts.Plex, 13).X > (dropdown.AbsoluteSize.X - 6) then
                    cutobject = true
                    table_remove(textchosen, #textchosen)
                end
            end
        ...
        if not max then
            ...
            if table_find(content, option) then
                chosen = option
    ...
    function dropdowntypes:remove(option)
        if optioninstances[option] then
            ...
            if max then
                if table_find(chosen, option) then
                    table_remove(chosen, table_find(chosen, option))
    ...
    function window_table:create_page(cfg)
        ...
        function page_table:new_section(cfg)
            ...
            function section_table:new_container(cfg)
                ...
                function container_tbl:new_container(cfg)
                    ...
                    function multi_container_tbl:new_slider(cfg)
                        return library.object_slider(multi_multicontent, cfg)
                    end;
                    -- create dropdown
                    function multi_container_tbl:new_dropdown(cfg)
                        ...
                        default = normalizeDropdownDefault(default, dropdown_content, max)
                ...
                function container_tbl:new_slider(cfg)
                    return library.object_slider(multicontent, cfg)
                end;
                -- create dropdown
            ...
            function section_table:new_slider(cfg)
                return library.object_slider(content, cfg)
            end;
            -- create dropdown
